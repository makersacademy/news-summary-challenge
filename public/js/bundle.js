  ...
  var require_Summary = __commonJS({
    "src/Summary.js"(exports, module) {
      var Summary2 = class {
        ...
        getSectionArticles = (card2, section) => {
          fetch(`https://content.guardianapis.com/section?q=${section}&page-size=10&api-key=test&format=json&show-fields=body,headline,thumbnail`).then((response) => response.json()).then((data) => {
            const articles = data.response.results;
            articles.forEach((article) => {
              card2.createCard(article);
            });
