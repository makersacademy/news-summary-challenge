class Story {
  constructor(id, headline, thumbnail, summariser = ArticleSummariser) {
    this.id = id;
  ...
  async summary() {
    if (this.summarySentences === undefined) {
      ...
      let trimmedSentences = sentences.map((sentence) => {
        return sentence
          .replace(/\n/g, "")
          .replace(/EDT\d{2}:\d{2}/g, "")
          .replace(/Updatedat \d{2}.\d{2}\w{2}/, "");
      });
  ...
  async summaryComponent() {
    ...
    let articleLinkButton = this.createLinkButton(
