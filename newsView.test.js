...
const { runInThisContext } = require('vm');
 ...
 require('jest-fetch-mock').enableMocks()
  ...
  describe('newsView class', () => {
    ...
    it('displays a thumbnail for each headline', () => {
      const model = new NewsModel
      const fakeApi = {
        loadHeadlines: () => {
          model.setHeadlines([
            {
              "fields": {
                "headline": "A Live Headline",
                "thumbnail": "http://example.com/ExampleImage.jpg"
              }
            }
          ])
          view.displayHeadlines();
      ...
      expect(document.body.querySelectorAll('div.headline').length).toEqual(1)
      expect(document.body.querySelectorAll('img.thumbnail').length).toEqual(1)
      expect(document.body.querySelectorAll('div.headline')[0].innerText).toEqual("A Live Headline")
      expect(document.body.querySelectorAll('img.thumbnail')[0].src).toEqual("http://example.com/ExampleImage.jpg")
    })
  })